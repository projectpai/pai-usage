// Package btcutils contains a number of useful Bitcoin related functions originally used in the go-paicoin-multisig project
package btcutils
...
// secp256k1 curve as this is fairly specific to Paicoin.
...
// Hash160 performs the same operations as OP_HASH160 in Paicoin Script
...
func NewMOfNRedeemScript(m int, n int, publicKeys [][]byte) ([]byte, error) {
	...
	if n < 1 || n > 7 {
		return nil, errors.New("N must be between 1 and 7 (inclusive) for valid, standard P2SH multisig transaction as per Paicoin protocol.")
	}
	if m < 1 || m > n {
...
// NewRawTransaction creates a Paicoin transaction given inputs, output satoshi amount, scriptSig and scriptPubKey
